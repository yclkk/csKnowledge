...
每一个状态$f(i,j)$都表示的是一个集合，需要考虑的是$f(i,j)$表示的是哪一个集合，其中$f(i,j)$表示的是一个集合，但是存的是一个数，**这个数是这个集合的某种属性**

属性：$Max, Min$，数量

![dp](../src/algorithm/dp.png)
...
就是把当前这个集合划分为更小的子集，使得每一个子集都可以算出来，都可以用前面更小的状态计算出来

原则

1. 不重：一个元素当前只会属于某一个集合（不一定会满足）
2. 不漏：某个元素不会不属于任何一个集合

步骤：

1. 把$f(i,j)$表示的所有选法分为两大类
   - 选法不包含第$i$个物品
   - 选法包含第$i$个物品
2. 对于选法不包含第$i$个物品：从$[1,i)$ 中选，并且总体积不超过$j$，等价于从$[1,i-1]$中选，并且总体积不超过$j$，因此也就等价于$f(i-1,j)$
3. 对于选法包含第$i$个物品：如果说给全班同学都加50分，那么相对名次是不会发生变化。

   因此可以将选法中的第$i$个物品去掉，因此就可以转换为从$[1, i-1]$中选，并且总体积不超过$j-v_i$ ，也就等价于$f(i-1, j-v_i) + w_i$ 
4. 所以$f(i, j) = max(f(i-1,j), f(i-1, j-v_i) + w_i)$

![dp_01backpack](../src/algorithm/dp_01backpack.png)

![dp_01backpack_1](../src/algorithm/dp_01backpack_1.png)

```c++
// 两重循环
cin >> n >> m;
for (int i = 1; i <= n; i++) cin >> v[i] >> w[i];
// f[0][0..m] = 0，含义是在前0个物品选，所以属性为0
for (int i = 1; i <= n; i++) {
  for (int j = 0; j <= m; j++) {
    f[i][j] = f[i - 1][j];
    // 不一定会包含i，只有当v[i]<=j的时候才有可能包含i
...
1. $f(i)$只用到了$f(i-1)$这一层，因此可以变为滚动数组
2. 对于$f(i, j) = max(f(i-1,j), f(i-1, j-v_i) + w_i)$ 来说，$j$,$j-v_i$都$\leq j$
3. 在算$f(j)$的时候，$f(j-v_i)$在第$i$层已经被计算过了
...
for (int i = 1; i <= n; i++) {
  // 因为if(j >= v[i])才会计算，因此可以优化j的初始值
  //for (int j = v[i]; j <= m; j++) {
  for (int j = m; j >= v[i]; j--) {
    // f[j] = f[j]可以删掉
    /*
     由于j是从小到大的，而j-v[i]<=j，所以f[j - v[i]]是在当前第i层就被计算过了，
     因此f[j] = max(f[i][j], f[i][j - v[i]] + w[i]) 是不对的
 		 所以改第二层for循环，j从大到小，因此f[j - v[i]]在当前第i层没被计算过，使用的是i-1层，
 		*/
